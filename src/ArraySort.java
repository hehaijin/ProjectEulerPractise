/**
 * Practicing codes for algorithms of sorting on a int array
 * 
 * @author Haijin He
 *
 */
public class ArraySort
{
  ...
  /**
   * randomly generate a array of given size. the values are from 0-99
   * 
   * @param size
  ...
  public static void heapify(int array[], int index, int heapsize)
  {
    ...
    if (left < heapsize && array[left] > array[index])
      larger = left; // compare left with itself
    if (right < heapsize && array[right] > array[larger])
      larger = right; // compare right with itself
  ...
  */
  ...
  /**
   * the real Quicksort recurrence function
   * @param array
   * @param left
   * @param right
   */
